  ...
  if (contracts.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
        <p className="text-gray-500">No contracts available. Add apps and complete their details to see them here.</p>
      </div>
    );
  }
          ...
          <div key={group.appId} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            ...
            <div className="px-6 py-4">
              <div className="space-y-6">
                ...
                {(group.notes || group.contactDetails) && (
                  <div className="grid grid-cols-2 gap-6">
                    {group.notes && (
                      <div>
                        <label className="block text-sm font-medium text-gray-500">Notes</label>
                        <p className="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{group.notes}</p>
                      </div>
                    )}
                    {group.contactDetails && (
                      <div>
                        <label className="block text-sm font-medium text-gray-500">Contact Details</label>
                        <p className="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{group.contactDetails}</p>
                      </div>
                    )}
                  </div>
                )}
